...
export async function PUT(
  ...
  try {
    ...
    const url = `${base}/api/v1/conversations/${encodeURIComponent(
      convId
    )}/messages/${encodeURIComponent(msgId)}/reaction`;
    ...
    console.error('[api/conversations] reaction proxy error', error);
    return NextResponse.json({ error: 'Internal reaction proxy error' }, { status: 500 });
  }
}
