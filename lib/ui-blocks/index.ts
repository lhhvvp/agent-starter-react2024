...
export const LK_UI_ACKS_TOPIC = 'lk.ui.acks' as const;
...
export type UIBlockBase = {
  id: string;
  type: string;
  state?: { loading?: boolean; disabled?: boolean; reason?: string };
};

export type UITextBlock = UIBlockBase & {
  type: 'text';
  content: string;
  variant?: 'muted' | 'body' | 'title' | 'subtitle';
  format?: 'plain' | 'md';
};
...
export type UICardBlock = UIBlockBase & {
  type: 'card';
  title?: string;
  subtitle?: string;
  body: UIBlock[];
...
export type UIFormBlock = UIBlockBase & {
  type: 'form';
  title?: string;
  fields: UIFormField[];
  submit: { label?: string; action: UIToolAction };
};
...
export type UIEvent =
  ...
  | {
      ...
      args: {
        ...
        ui?: { text?: string; lang?: string; blocks: UIBlock[] };
      };
    }
  | {
      name: 'tool.error';
  ...
  | {
      name: 'ui.error';
      args: { requestId: string; messageId: string; code: string; message: string };
    };
